...
if check_index_files(list_dir, index_filenames) == False:
    align = Mapping(dic_args['reference'], dic_args['reads'], dic_args['output'])
    align.index()
...
if sum_not_bam(nb) > 0: # if not all items are .bam
    for nb_elem in nb:
        to_map = Mapping(dic_args['reference'], nb_elem[0], dic_args['output'])
        #to_map.map()
        dic_args['reads'] = [read.replace('.sam','.bam') for read in dic_args['reads']]

# next: fixmate and sort
# We have two instances of Mapping at times... way to only have one instance?
